...
export default function App() {
  ...
  const handleRequestSubmit = async () => {
    ...
    try {
      ...
      notifyLine(`🔔 มีคำขอเบิกใหม่\nโดย: ${newReq.employeeName}\nยอด: ฿${total.toLocaleString()}\nID: ${id}`);
    ...
    try {
      ...
      if (systemConfigs.sheetsUrl) {
         syncToSheets(systemConfigs.sheetsUrl, {
            ...
            total: total,
            requested: selectedAdvData.totalAmount,
            balance: selectedAdvData.balance - total,
            clearedAt: new Date().toISOString(),
            status: 'Cleared'
         });
      }
            ...
            <div className="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm space-y-4">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                <div className="space-y-1.5">
                  <label className="text-[8px] font-black text-slate-400 uppercase tracking-widest px-1">Employee</label>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div className="bg-white p-6 rounded-[2.5rem] border border-slate-100 shadow-sm space-y-4 overflow-hidden">
                ...
                <div className="space-y-3 max-h-[300px] overflow-y-auto pr-2">
                  {dashboardData.list.slice(0, 10).map((r, i) => (
                    <div key={i} className="flex justify-between items-center border-b border-slate-50 pb-2 last:border-0 last:pb-0">
                      <div>
                        <p className="text-[10px] font-bold text-slate-800 leading-none">{r.name}</p>
                        <p className="text-[8px] font-black text-slate-300 uppercase mt-1">{r.projectId} • {r.employeeName}</p>
                      </div>
                      <p className="text-[11px] font-black text-blue-600">฿{r.amount.toLocaleString()}</p>
                    </div>
                  ))}
            ...
            <div className="bg-white p-8 rounded-[2.5rem] border border-slate-100 shadow-sm space-y-6">
              ...
              <div className="space-y-4">
                ...
                <div className="p-4 bg-blue-50 rounded-2xl border border-blue-100">
                  <p className="text-[8px] font-bold text-blue-700 leading-relaxed">
                    💡 <span className="font-black">วิธีใช้งานสำหรับฝ่ายบัญชี:</span> เปิด Google Sheet {"→"} ส่วนขยาย {"→"} App Script แล้ววางโค้ดที่เตรียมไว้ จากนั้นนำ Webhook URL มาใส่ที่นี่ ข้อมูลการเคลียร์เงินจะถูกส่งเข้า Excel ทันที!
                  </p>
                </div>
              ...
              <div className="border-t border-slate-50 pt-6 space-y-3">
                <div className="flex justify-between items-center">
                  <div>
                    <p className="text-[11px] font-black text-slate-900 uppercase">LINE Notify</p>
                    <p className="text-[8px] font-bold text-slate-400 uppercase tracking-widest mt-0.5">Status: Configured in System</p>
                  </div>
                  <button onClick={() => notifyLine("🧪 Test System: การแจ้งเตือนทำงานปกติ!")} className="bg-white border border-slate-200 text-slate-900 px-4 py-2 rounded-xl font-black text-[9px] uppercase tracking-widest hover:bg-slate-100 active:scale-95 transition-all shadow-sm">Test Notification</button>
                </div>
              </div>
            </div>
          ...
          <div className="bg-white w-full max-w-2xl sm:rounded-[3rem] rounded-t-[3rem] p-6 sm:p-10 shadow-2xl animate-in slide-in-from-bottom-full duration-500 overflow-y-auto max-h-[95vh]">
            <div className="flex justify-between items-start mb-8">
               <div className="space-y-1">
                 ...
                 <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">{selectedWithdrawal.advanceId}</p>
               </div>
